...
<div class="box-body">
    ...
    <table id="dataList" class="table table-bordered table-striped table-hover dataTable text-center">
        ...
        <tbody>
        <c:forEach items="${pageResult}" var="book">
            <tr>
                <td> ${book.name}</td>
                <td>${book.author}</td>
                <td>${book.press}</td>
    ...
    <button type="button" class="btn bg-olive btn-xs" onclick="prePage(${pageNum},${pages})">上一页</button>&nbsp;${pageNum}&nbsp;
    <button type="button" class="btn bg-olive btn-xs" onclick="nextPage(${pageNum},${pages})">下一页</button>
    <!-- 数据表格 /-->
</div>
<!-- 数据展示内容区/ -->
<%--引入存放模态窗口的页面--%>
...
<script>
    function prePage(index,pages) {
        if(index>1) index=index-1;
        else index=1;
        var url="/book/selectPageByIndex?index="+index;
        $.get(url,function (response) {
            console.log(response);
        });
    }
    function nextPage(index,pages) {
